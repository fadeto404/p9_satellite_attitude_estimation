classdef MM_MEKF_CAL
    properties
        ...
        GQG {mustBeNumeric}         % G*Q*G' (continuous time)
        Q_k {mustBeNumeric}         % Discretized spectral density
        Q_kk {mustBeNumeric}        % G*Q_k*G'
        S_hat {mustBeNumeric}       % Calibrated gyro scaling and misalignment parameters
        ISinv {mustBeNumeric}       % inv(eye(3) + S_hat)
    ...
    methods
        function obj=MM_MEKF_CAL(dx0, P0, q0, omega0, F, H, G, Q, R, t_s, S_hat)
            ...
            obj.GQG = obj.G*obj.Q*obj.G';
            obj.S_hat = S_hat;
            obj.ISinv = inv(eye(3) + obj.S_hat);
        end
        function obj=propagate(obj)
            % Propagate reference quaternion using last gyro reading as
            % angular velocity
            obj.omega_est = obj.ISinv*(obj.omega - obj.beta_g);
...
%             obj.F = [-Omega, -obj.ISinv; zeros(3,6)];
%             script_B = expm([-obj.F, obj.GQG; zeros(6,6), obj.F']*obj.dt);
%             Phi = script_B(7:end,7:end)';
%             obj.Q_kk = Phi*script_B(1:6,7:end);
            
%             obj.F = [-Omega, -diag(obj.omega - obj.beta_g); zeros(3,6)]; 
%             Phi = eye(6) + obj.F*obj.dt;
%             obj.Q_kk = obj.dt*obj.GQG;
