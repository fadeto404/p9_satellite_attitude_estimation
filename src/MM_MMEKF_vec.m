classdef MM_MMEKF_vec
    ...
    methods
        function obj=MM_MMEKF_vec(dx0, P0, q0, omega0, nmax, F, H, G, Q, R, t_s)
...
%             obj.Pxx_pre = obj.Pxx_post + Pxx_dot*obj.delta_t; % Discretize
        ...
        function obj=update(obj, z)
            ...
            for j = 1:length(row)
                ...
                obj.Pxx_post = (eye(6)-obj.K*obj.H)*obj.Pxx_pre*(eye(6)-obj.K*obj.H)'+obj.K*obj.R*obj.K'; % Update error state covariance (Joseph form for stability)

                obj.dx_post = obj.dx_pre + obj.K*(b_meas - b_meas_est); % Update error states

                if j < length(row)
                    obj.Pxx_pre = obj.Pxx_post;
                    obj.dx_pre = obj.dx_post;
                end
            end
